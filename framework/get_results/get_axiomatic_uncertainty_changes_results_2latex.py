...
def get_axiomatic_results(args):
    ...
    def run_axiomatic_metrics(prompt_order, axiomatic_evalator, uncertainty_type, uncertainty_model):
        ...
        if uncertainty_type == 'calibrated':
            ...
            filtered_df = result_df_main_prompt[result_df_main_prompt[f'axiom_num_{axiomatic_evalator}'].isin(['1', '2', '4', '5', 'others'])]
            ...
            result = result_df_main_prompt_filtered.groupby(f'axiom_num_{axiomatic_evalator}').agg(
        ...
        for axiom_num in ['3']:
            ...
            # === Get samples =======
            if axiom_num in ['1', '2', '4', '5', 'others']:
                selected_main_prompt_df = result_df_main_prompt[result_df_main_prompt[f'axiom_num_{axiomatic_evalator}'] == axiom_num]
                selected_second_prompt_df = result_df_second_prompt[result_df_second_prompt['id'].isin(selected_main_prompt_df['id'].tolist())]
            elif axiom_num == '3':
                selected_main_prompt_df = result_df_main_prompt[result_df_main_prompt['output_equality_em'] == True]
                selected_second_prompt_df = result_df_second_prompt[result_df_second_prompt['id'].isin(selected_main_prompt_df['id'].tolist())]
            ...
            # === Get Uncertainty ===
            uncertainty_values_main_prompt = selected_main_prompt_df[f"{unc_model_key_main_prompt}_cal"] if uncertainty_type == 'calibrated' else selected_main_prompt_df[unc_model_key_main_prompt]
            ...
            axiom_outputs.append((
                f"{uncertainty_values_second_prompt_filtered.mean():.3f}",
                f"{uncertainty_values_main_prompt_filtered.mean():.3f}",
                is_significant
            ))
    ...
    axiomatic_evalator = 'correctness' # 'correctness', 'minicheck', 'nli', 'kldiv'
    uncertainty_type = 'normal' # 'normal', 'calibrated' 
    ...
    for uncertainty_model in ['PE', 'SE', 'PE_MARS', 'SE_MARS', 'spectral_u', 'ecc_u', 'degree_u']: 
        dict_output[uncertainty_model] = run_axiomatic_metrics(prompt_order, axiomatic_evalator, uncertainty_type, uncertainty_model)
...
if __name__ == "__main__":
    ...
    for axiom_title, axiom_values in dict2save.items():
        ...
        for unc_title, unc_values in axiom_values.items():
            ...
            for un_idx, unc_value in enumerate(unc_values):
                ...
                sign_mark = r"$\ast$" if unc_value[2] else " "
                color, density = get_color(unc_value, axiom_title)
                if un_idx == len(unc_values)-1:
                    latex_table += fr"\colorbox{{{color}!{density}}}{{{unc_value[0]} $\rightarrow$ {unc_value[1]}~\textsuperscript{{{sign_mark}}}}} \\"
                else:
                    latex_table += fr"\colorbox{{{color}!{density}}}{{{unc_value[0]} $\rightarrow$ {unc_value[1]}~\textsuperscript{{{sign_mark}}}}} &"
    ...
    # python framework/get_results/get_axiomatic_uncertainty_changes_results_2latex.py
